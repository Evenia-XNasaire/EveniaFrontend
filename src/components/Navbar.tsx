...
interface NavbarProps {
    onDashboardMobileToggle?: () => void;
}

const Navbar: React.FC<NavbarProps> = React.memo(({ onDashboardMobileToggle }) => {
    ...
    return (
        <nav
            className="fixed top-0 left-0 right-0 z-50 w-full bg-[var(--surface)] border-b border-[var(--glass-border)] shadow-md"
            style={{
        ...
        >
            <div className="max-w-6xl mx-auto px-4 sm:px-6 md:px-8 h-14 sm:h-16 flex items-center justify-between">
                {/* Left: Logo & Nav Links */}
                <div className="flex items-center gap-1 sm:gap-12 shrink-0">
                    <Link to="/" className="flex items-center gap-1 sm:gap-2 group">
                        <img
                            src="/logo.png"
                            alt="Evenia Ticket Logo"
                            className="w-8 h-8 sm:w-14 sm:h-14 object-contain group-hover:scale-110 transition-transform duration-300"
                        />
                        <span className="text-sm sm:text-2xl font-black tracking-tighter truncate">
                ...
                <div className="flex items-center gap-1 sm:gap-4 shrink-0">
                    <button
                        onClick={toggleTheme}
                        className="p-1.5 sm:p-2.5 rounded-none bg-[var(--surface)] text-[var(--text-muted)] hover:text-primary hover:bg-primary/10 transition-all border border-[var(--border)] cursor-pointer"
                    >
                    ...
                    {user && (
                        <div className="relative">
                            <button
                                ...
                                className="p-1.5 sm:p-2.5 rounded-none bg-[var(--surface)] text-[var(--text-muted)] hover:text-primary hover:bg-primary/10 transition-all border border-[var(--border)] cursor-pointer relative"
                            >
                                <Bell className="w-4 h-4 sm:w-5 sm:h-5" />
                                {unreadCount > 0 && (
                                    <span className="absolute -top-1 -right-1 min-w-[14px] h-[14px] sm:min-w-[18px] sm:h-[18px] bg-danger text-white text-[7px] sm:text-[10px] font-black rounded-full flex items-center justify-center border-2 border-[var(--background)] px-0.5">
                            ...
                            <AnimatePresence>
                                {isNotificationsOpen && (
                                    <motion.div
                                        ...
                                        className="fixed inset-x-4 top-[70px] sm:absolute sm:inset-auto sm:right-0 sm:top-full sm:mt-3 sm:w-96 bg-[var(--surface)] border border-[var(--border)] rounded-none shadow-2xl overflow-hidden z-50 origin-top sm:origin-top-right"
                                    >
                    ...
                    {user ? (
                        <div className="flex items-center gap-1 sm:gap-4">
                            ...
                            <Link to={user.role === 'organizer' ? '/organizer' : '/profile'} className="w-8 h-8 sm:w-10 sm:h-10 rounded-none bg-linear-to-br from-primary to-accent border-2 border-white/20 flex items-center justify-center text-white font-black shadow-lg hover:scale-105 transition-transform">
                                {user.first_name[0]}
                            </Link>
                            <button onClick={handleLogout} className="p-1 sm:p-2 text-danger/70 hover:text-danger hover:scale-110 transition-all cursor-pointer">
                                <LogOut className="w-4 h-4 sm:w-5 sm:h-5" />
                            </button>
                        </div>
                    ) : (
                        <div className="hidden lg:flex items-center gap-2">
                            <Link to="/login" className="text-xs sm:text-sm font-bold text-[var(--text-muted)] hover:text-primary transition-colors px-2 sm:px-3 py-2 border border-transparent hover:border-[var(--border)] rounded-none">Connexion</Link>
                            <Link to="/register" className="btn-primary py-2 sm:py-2.5 px-4 sm:px-6 rounded-none text-xs sm:text-sm font-bold transition-all whitespace-nowrap">S'inscrire</Link>
                        </div>
                    )}

                    {/* Mobile Toggle */}
                    <button
                        className="lg:hidden p-1.5 rounded-none bg-[var(--surface)] border border-[var(--border)] text-primary shadow-xs active:scale-95 transition-all"
                                ...
                                <>
                                    ...
                                    >
                                        <Link
                                            to={user.role === 'organizer' ? '/organizer' : '/my-tickets'}
                                            onClick={(e) => {
                                                if (onDashboardMobileToggle) {
                                                    e.preventDefault();
                                                    onDashboardMobileToggle();
                                                }
                                                setIsMenuOpen(false);
                                            }}
