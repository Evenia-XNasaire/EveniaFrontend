...
const EventDetailsPage: React.FC = () => {
    ...
    return (
        <MainLayout>
            ...
            {/* Hero section - Full Width and Flushed to Navbar */}
            <motion.section
                initial={{ opacity: 0, y: 30 }}
                animate={{ opacity: 1, y: 0 }}
                className="relative h-[400px] sm:h-[550px] overflow-hidden shadow-2xl group"
            >
                {/* Floating Back Button Inside Image */}
                <button
                    onClick={() => navigate('/')}
                    className="absolute top-3 left-3 z-40 flex items-center gap-1.5 px-3 py-1.5 bg-black/40 backdrop-blur-md text-white rounded-none border border-white/10 hover:bg-black/60 transition-all font-bold text-[10px] sm:text-sm shadow-xl active:scale-95"
                >
                    ...
                    <>
                        <div className="absolute inset-0 z-30 flex items-center justify-between px-4 pointer-events-none">
                            <button
                                ...
                                className="w-8 h-8 sm:w-12 sm:h-12 bg-black/30 backdrop-blur-md hover:bg-black/50 text-white rounded-full flex items-center justify-center transition-all pointer-events-auto shadow-xl"
                            >
                        ...
                        <div className="absolute bottom-4 left-0 right-0 z-30 flex justify-center gap-1.5 px-4 overflow-x-auto pb-1 custom-scrollbar">
                            {allImages.map((img, idx) => (
                                <button
                                    key={idx}
                                    onClick={() => setCurrentImageIndex(idx)}
                                    className={`relative shrink-0 w-6 h-6 sm:w-12 sm:h-12 rounded-none overflow-hidden border transition-all hover:scale-105 active:scale-95 ${idx === currentImageIndex ? 'border-primary ring-1 ring-primary/20 scale-105' : 'border-white/20 hover:border-white/60'
                                        }`}
                                >
                ...
                <div className="absolute bottom-0 left-0 right-0 h-1/3 bg-linear-to-t from-black/80 to-transparent z-10" />

                <div className="absolute bottom-0 left-0 right-0 p-3 sm:p-6 z-50 space-y-1 sm:space-y-2">
                    <div className="flex flex-wrap gap-1.5 sm:gap-2 items-center">
                        <span className="px-2 sm:px-4 py-1 bg-primary text-white text-[8px] sm:text-xs font-black uppercase tracking-widest rounded-none">
                            {event.category}
                        </span>
                        {event.event_status === 'Passé' && (
                            <span className="px-2 sm:px-4 py-1 bg-danger text-white text-[8px] sm:text-xs font-black uppercase tracking-widest rounded-none">
                                Terminé
                            </span>
                        )}
                    </div>

                    <h1 className="text-lg sm:text-3xl font-black text-white tracking-tight leading-tight max-w-[90%]">
                        {event.title}
                    </h1>

                    <div className="flex flex-wrap gap-2 sm:gap-6 items-center text-white/90">
                        <div className="flex items-center gap-1 sm:gap-1.5">
                            <Calendar className="text-primary w-3 h-3 sm:w-4 sm:h-4" />
                            <p className="font-bold text-[9px] sm:text-xs">{format(eventDate, 'd MMM yyyy HH:mm', { locale: fr })}</p>
                        </div>

                        <div className="flex items-center gap-1 sm:gap-1.5">
                            <MapPin className="text-primary w-3 h-3 sm:w-4 sm:h-4" />
                            <p className="font-bold text-[9px] sm:text-xs">{event.location}</p>
                        </div>

                        <div className="w-full sm:hidden" /> {/* Force break on mobile if needed or just keep row */}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 items-start">
                    ...
                    <div className="lg:col-span-3 space-y-6 lg:max-h-[calc(100vh-150px)] lg:overflow-y-auto lg:pr-4 custom-scrollbar">
                        ...
                        <div className="card-surface p-6 sm:p-8 space-y-6">
                            <div className="flex items-center gap-4">
                                <div className="w-10 h-10 sm:w-14 sm:h-14 bg-primary/10 text-primary rounded-none flex items-center justify-center">
                                    <Info className="w-6 h-6 sm:w-8 sm:h-8" />
                                </div>
                                <div>
                                    <h3 className="text-xl sm:text-2xl font-black">À propos</h3>
                                    <p className="text-[10px] sm:text-sm text-[var(--text-muted)] font-medium uppercase tracking-widest">Informations sur l'événement</p>
                                </div>
                            </div>

                            <div className="p-4 sm:p-8 bg-[var(--background)]/40 border border-[var(--border)] shadow-inner">
                            ...
                            {user ? (
                                <div className="space-y-4">
                                    <textarea
                                        ...
                                        className="w-full bg-[var(--background)] border border-[var(--border)] p-4 font-medium outline-none focus:border-primary transition-all min-h-[100px]"
                                    />
                                    <button
                            ...
                            <div className="space-y-4">
                                {event.comments.map((comment) => (
                                    <div key={comment.id} className="space-y-2">
                                        ...
                                        <p className="text-sm text-[var(--text-muted)] bg-[var(--background)] p-4 border border-[var(--border)]">
                                            {comment.content}
                                        </p>
                                    </div>
                                ))}
                            ...
                            <div className="card-surface p-6 sm:p-8 space-y-6">
                                ...
                                <div className="space-y-4">
                                    {event.ticket_types.map((type) => (
                                        <div key={type.id} className="p-5 bg-[var(--background)] rounded-none border border-[var(--border)] relative group hover:border-primary/30 transition-all space-y-4">
                                            <div className="flex justify-between items-start gap-4">
                                                <div className="flex-1">
                                                    <h4 className="font-black text-xl text-primary">{type.name}</h4>
                                                ...
                                                <div className="text-right">
                                                    <span className="text-2xl font-black text-primary block">{Math.round(type.price).toLocaleString()} FCFA</span>
                                            ...
                                            {type.benefits && (
                                                <div className="p-4 bg-white/60 rounded-none border border-[var(--border)]/40 shadow-sm">
                                                    <p className="text-[10px] font-black uppercase text-primary mb-2 tracking-wider">Avantages & Bénéfices :</p>
                                            ...
                                            <div className="flex items-center justify-between pt-4 border-t border-[var(--border)]/10 mt-auto">
                                                <div className="flex flex-col">
                                                    <span className="text-[10px] font-black text-[var(--text-muted)] uppercase tracking-tighter">Disponibilité</span>
                                                    <span className="text-xs font-bold text-primary">
                                                        {type.quantity_available} billets restants
                                                    </span>
                                                </div>
                                                <div className="flex items-center gap-3 bg-[var(--surface)] p-1.5 border border-[var(--border)]">
                                                    <button
                                                        onClick={() => handleQuantityChange(type.id, -1)}
                                                        disabled={!selectedTickets[type.id] || event.event_status === 'Passé'}
                                                        className="w-8 h-8 flex items-center justify-center bg-[var(--background)] border border-[var(--border)] hover:border-primary transition-all disabled:opacity-30"
                                                    >
                                                        <Minus size={14} />
                                                    </button>
                                                    <span className="w-8 text-center font-black text-lg">{selectedTickets[type.id] || 0}</span>
                                                    <button
                                                        onClick={() => handleQuantityChange(type.id, 1)}
